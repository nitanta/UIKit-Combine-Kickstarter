...
public extension Date {
    ...
    // MARK: FIND THE DIFFERENCE BETWEEN TWO DATES
    ...
    func getDifferenceInHours(sinceDate: Date) -> Int? {
        return Calendar.current.dateComponents([.hour], from: sinceDate, to: self).hour
    }
    
    func getDifferenceInDays(sinceDate: Date) -> Int? {
    ...
    /// Converts the date to string based on a relative time language. i.e. just now, 1 minute left etc...
    func toStringWithRelativeTime(strings:[RelativeTimeStringType:String]? = nil) -> String {
        ...
        if sec < 60 {
            if sec < 10 {
                if isPast {
                    ...
                    return strings?[.nowFuture] ?? NSLocalizedString("few seconds left", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    ...
                    string = strings?[.secondsFuture] ?? NSLocalizedString("%.f seconds left", comment: "Date format")
                }
        ...
        if min < 60 {
            if min == 1 {
                if isPast {
                    ...
                    return strings?[.oneMinuteFuture] ?? NSLocalizedString("1 minute left", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    ...
                    string = strings?[.minutesFuture] ?? NSLocalizedString("%.f minutes left", comment: "Date format")
                }
        ...
        if hr < 24 {
            if hr == 1 {
                if isPast {
                    ...
                    return strings?[.oneHourFuture] ?? NSLocalizedString("1 hour left", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    ...
                    string = strings?[.hoursFuture] ?? NSLocalizedString("%.f hours left", comment: "Date format")
                }
        ...
        if d < 7 {
            if d == 1 {
                if isPast {
                    ...
                    return strings?[.oneDayFuture] ?? NSLocalizedString("1 day left", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
                    ...
                    string = strings?[.daysFuture] ?? NSLocalizedString("%.f days left", comment: "Date format")
                }
        ...
        if d < 28 {
            if isPast {
                ...
                if compare(.isNextWeek) {
                    return strings?[.oneWeekFuture] ?? NSLocalizedString("1 week left", comment: "Date format")
                } else {
                    let string = strings?[.weeksFuture] ?? NSLocalizedString("%.f weeks left", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .week))))
                }
            }
        }
        if compare(.isThisYear) {
            if isPast {
                ...
                if compare(.isNextMonth) {
                    return strings?[.oneMonthFuture] ?? NSLocalizedString("1 month left", comment: "Date format")
                } else {
                    let string = strings?[.monthsFuture] ?? NSLocalizedString("%.f months left", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .month))))
                }
            }
        }
        if isPast {
            ...
            if compare(.isNextYear) {
                return strings?[.oneYearFuture] ?? NSLocalizedString("1 year left", comment: "Date format")
            } else {
                let string = strings?[.yearsFuture] ?? NSLocalizedString("%.f years left", comment: "Date format")
                return String(format: string, Double(abs(since(Date(), in: .year))))
            }
        }
    }
    
    /// Converts the date to string based on a relative time language. i.e. just now, in 1 minute etc...
    func toStringWithRelativeTimeFormat(strings:[RelativeTimeStringType:String]? = nil) -> String {
        ...
        if sec < 60 {
            if sec < 10 {
                if isPast {
                    ...
                    return strings?[.nowFuture] ?? NSLocalizedString("in few seconds left", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
        ...
        if hr < 24 {
            if hr == 1 {
                if isPast {
                    ...
                    return strings?[.oneHourFuture] ?? NSLocalizedString("in 1 hour", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
        ...
        if d < 7 {
            if d == 1 {
                if isPast {
                    ...
                    return strings?[.oneDayFuture] ?? NSLocalizedString("in 1 day", comment: "Date format")
                }
            } else {
                let string:String
                if isPast {
        ...
        if d < 28 {
            if isPast {
                if compare(.isLastWeek) {
                    ...
                    let string = strings?[.weeksPast] ?? NSLocalizedString("in %.f weeks", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .week))))
                }
            } else {
                if compare(.isNextWeek) {
                    return strings?[.oneWeekFuture] ?? NSLocalizedString("in 1 week", comment: "Date format")
                } else {
        ...
        if compare(.isThisYear) {
            if isPast {
                if compare(.isLastMonth) {
                    ...
                    let string = strings?[.monthsPast] ?? NSLocalizedString("in %.f months", comment: "Date format")
                    return String(format: string, Double(abs(since(Date(), in: .month))))
                }
            } else {
                if compare(.isNextMonth) {
                    return strings?[.oneMonthFuture] ?? NSLocalizedString("in 1 month", comment: "Date format")
                } else {
        ...
        if isPast {
            ...
            if compare(.isNextYear) {
                return strings?[.oneYearFuture] ?? NSLocalizedString("in 1 year", comment: "Date format")
            } else {
